...
namespace Gemini.Controllers.Combo
{
    public class ComboController : GeminiController
    {
        ...
        public JsonResult ReadGallery(string nameGroup)
        {
            List<UGalleryModel> uGalleryModel = (from ug in DataGemini.UGalleries
                                                 ...
                                                 where groups.Name.ToLower().Trim() == nameGroup.ToLower().Trim()
            ...
            return Json(uGalleryModel.Select(c => new { c.Guid, c.Name, c.Image }), JsonRequestBehavior.AllowGet);
        }

        public JsonResult TransportBy()
        {
            List<SUserModel> sUserModel = (from su in DataGemini.SUsers
                                           join sr in DataGemini.SRoles on su.GuidRole equals sr.Guid
                                           where sr.IsTransport == true
            ...
            return Json(sUserModel.Select(c => new { c.Guid, c.Username }), JsonRequestBehavior.AllowGet);
        }

        public JsonResult DistributeBy()
        {
            List<SUserModel> sUserModel = (from su in DataGemini.SUsers
                                           join sr in DataGemini.SRoles on su.GuidRole equals sr.Guid
                                           where sr.IsDistribute == true
                                           select new SUserModel
                                           {
                                               Guid = su.Guid,
                                               Username = su.Username,
